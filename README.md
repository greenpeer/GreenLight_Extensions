...
Matlab extensions for the [GreenLight - A model for greenhouses with supplemental lighting](#https://github.com/davkat1/GreenLight) , including `setParamVal` and `glObjToJson`.
...
4. The [`setParamVal`](#setParamVal) function changes the value of a parameter in a `DynamicModel` object. It throws an error if the `DynamicElement` of `dm.x.<name>` doesn't exist.
